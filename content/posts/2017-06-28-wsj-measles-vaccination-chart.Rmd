...
Finally, we'll create our measles data frame by summarizing the incidence of measles occuring in each state by year, while also recoding instances in which state's have no incidence/values for a given year as NAs.
...
It's dang easy to create an animation using `gganimate`. Just add `frame` to the initial ggplot2 call and you're good to go! In our plot, we'll want to interate over the data years, so `.frame = year`
...
```{r gganimate}
gganimate(gg, "/home/michael/Documents/hugo/static/img/gganimate_measles.gif")
```
...
The main idea behind creating our animation will be iteratively print images of larger subsets of our data set. In the for-loop, `subset(mdf, year<=1928+i-1)` takes the i-th value to subset the data year used in the figure. For example, for i==5, the data years 1928 through 1932 will appear in that image.
...
saveGIF(
  ...
  ,movie.name="/home/michael/Documents/hugo/static/img/measles.gif",interval = .1, ani.width = 2050, ani.height = 1550)
```
