...
**UPDATE**: In order to reproduce some portions of this code you need the `gganimate` package using the deprecated API. To access that version, download v0.1.1 [here](https://github.com/thomasp85/gganimate/releases/tag/v0.1.1).
...
One piece I wanted to replicate was the state abbreviations used in the figure. I used the [AP style guide](https://en.wikipedia.org/wiki/List_of_U.S._state_abbreviations), and you can access that csv through [this link](https://mikelee.co/data/stateName.csv):
```{r stateNames, eval = FALSE}
...
```{r measles data frame, eval = FALSE}
...
```{r colors, message=FALSE,warning=FALSE, eval = FALSE}
...
```{r gganimate prep, echo=FALSE, eval = FALSE}
...
Customized gifs can get super large. The version above tops of at 73.4 MB! We'll need to reduce that size for . One source for an #rstats solution is [this site](http://stla.github.io/stlapblog/posts/AnimatedGifs.html) - which uses the open source tool [gifsicle](https://github.com/kohler/gifsicle) to reduce gif file sizes. Let's port this author's `gif_compress` script to minimize our measles gif:
